...
function AllSchedule(props) {
  ...
  useEffect(() => {
    ...
    axios
      .get(`${DOMAIN_API}:${PORT_API}/data/student/all-schedule`, {
  ...
  if (schedules) {
    classItems8 = schedules
      .filter(schedules => schedules.time === '08:00:00')
      .map(cl => {
        return (
    ...
    classItems11 = schedules
      .filter(schedules => schedules.time === '11:00:00')
      .map(cl => {
        return (
          <View style={styles.scheduleItem} key={cl.id}>
            <Text style={styles.scheduleName}>
    ...
    classItems13 = schedules
      .filter(schedules => schedules.time === '13:00:00')
      .map(cl => {
        return (
          <View style={styles.scheduleItem} key={cl.id}>
            <Text style={styles.scheduleName}>
    ...
    classItems15 = schedules
      .filter(schedules => schedules.time === '15:00:00')
      .map(cl => {
        return (
          <View style={styles.scheduleItem} key={cl.id}>
            <Text style={styles.scheduleName}>
